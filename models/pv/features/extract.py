...
Run ``python -m newspaper.models.pv.features.extract --months 2025-07 2025-01`` for a
...
def _agg_import_month(df: pd.DataFrame, year: int, month: int,
                      clear_days: set, clearness: dict) -> pd.DataFrame:
    df = df[df["mp_id"].notna()].copy()
    df["is_clear"] = df["date"].isin(clear_days)
    df["clearness"] = df["date"].map(clearness)
    g = df.groupby("mp_id")
    ...
    # sufficient statistics for OLS of midday_ratio (y) on radiation clearness (x),
    # pooled across all summer days in build_features_agg -> load_irr_slope/corr.
    r = df.dropna(subset=["clearness", "midday_ratio"]).copy()
    r["xy"] = r["clearness"] * r["midday_ratio"]
    r["xx"] = r["clearness"] ** 2
    r["yy"] = r["midday_ratio"] ** 2
    rg = r.groupby("mp_id")
    irr = pd.DataFrame({
        "irr_n": rg.size(), "irr_sx": rg["clearness"].sum(),
        "irr_sy": rg["midday_ratio"].sum(), "irr_sxy": rg["xy"].sum(),
        "irr_sxx": rg["xx"].sum(), "irr_syy": rg["yy"].sum(),
    })
...
def run(months: list[str] | None = None) -> None:
    ...
    clearness = dict(zip(weather["date"].to_numpy(), weather["clearness"].to_numpy()))
    ...
    for e in exports:
        ...
        if imp_parts:
            ...
            monthly_rows.append(
                _agg_import_month(imp, e.year, e.month, clear_days, clearness))
