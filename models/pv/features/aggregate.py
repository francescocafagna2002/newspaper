...
def _seasonal(m: pd.DataFrame) -> pd.DataFrame:
    ...
    a = agg(su, dict(
        ...
        irr_n=("irr_n", "sum"), irr_sx=("irr_sx", "sum"), irr_sy=("irr_sy", "sum"),
        irr_sxy=("irr_sxy", "sum"), irr_sxx=("irr_sxx", "sum"), irr_syy=("irr_syy", "sum"),
    ))
    b = agg(wi, dict(
    ...
    # ---- weather-solar coupling: OLS of midday_ratio on radiation clearness ----
    # (SunDance "Universal Weather-Solar Effect"): strongly negative for PV.
    n = out["irr_n"].where(out["irr_n"] >= 20)
    sxx_c = n * out["irr_sxx"] - out["irr_sx"] ** 2
    syy_c = n * out["irr_syy"] - out["irr_sy"] ** 2
    sxy_c = n * out["irr_sxy"] - out["irr_sx"] * out["irr_sy"]
    out["load_irr_slope"] = (sxy_c / sxx_c.where(sxx_c > 0)).clip(-5, 5)
    out["load_irr_corr"] = (sxy_c / np.sqrt((sxx_c * syy_c).where((sxx_c > 0) & (syy_c > 0))))
    out = out.drop(columns=["irr_n", "irr_sx", "irr_sy", "irr_sxy", "irr_sxx", "irr_syy"])
