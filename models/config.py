...
DOCS = NEWSPAPER / "docs"
ARTIFACTS = NEWSPAPER / "models" / "artifacts"
...
def discover_monthly_exports(root: Path = INPUT_DATA) -> list[MonthlyExport]:
    ...
    for dirpath, _dirs, files in os.walk(root):
        ...
        year = int(m.group(2))
        for fn in files:
            if fn.startswith("LG_AIM2Hackerdays_kWh_") and fn.endswith(".csv"):
                found[(year, month)] = MonthlyExport(
                    Path(dirpath) / fn, year, month
                )
    return sorted(found.values(), key=lambda e: (e.year, e.month))


# Known data-quality caveats (see docs/data_problems.md)
KNOWN_MISSING_MONTHS = {"2023-03"}          # empty folder, no export
