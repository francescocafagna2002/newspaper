...
| [`heuristics.py`](heuristics.py) | Rule-based **PV / EV / battery** presence baselines — the bar the trained models must clear. Physical rules, almost no fitted constants, confusion matrix for each. See [`../docs/heuristics_baseline.md`](../docs/heuristics_baseline.md). Run: `python -m newspaper.models.heuristics` (~45 s). |
