...
% #1 width, #2 the rule in one sentence, #3 its headline score.
% Flat: a single accent rule at the left edge, no box, per DESIGN_PREFERENCES.
% Plain LaTeX boxes, not a TikZ node -- a node with `text width` sets its own
% paragraph and spaces words far more loosely than body copy.
\newsavebox{\rbbox}
\newcommand{\rulebaseline}[3]{\par\smallskip\noindent
\sbox{\rbbox}{\parbox{#1}{\raggedright
  ...
  {\sffamily\scriptsize\bfseries\color{ink}#3}}}%
...
\confusiontable{\shortstack[l]{EV-labelled\\(GIGI)}}{\shortstack[l]{Unlabelled\\properties}}{10}{5}{20}{32}\\[4pt]
{\scriptsize\textcolor{muted}{$p \geq 0.392$; test $n=67$.}}
\rulebaseline{.92\linewidth}{Flag EV if grid import ever steps up by $\geq$\,9\,kW within one quarter-hour.}{ROC-AUC 0.686 \enspace n = 342}
\confusiontable{\shortstack[l]{EV-labelled\\(GIGI)}}{\shortstack[l]{Unlabelled\\properties}}{49}{23}{75}{195}
\end{columns}
\end{frame}

\begin{frame}{EV detection has a 2.3\,kW blind spot}
\begin{columns}[T,onlytextwidth]
\column{.56\textwidth}
\tagline{THE GAP IN THE SPEC}\par\vspace{.3cm}
\begin{itemize}\itemsep.5em
  \item L1 is 1.4--1.8\,kW. L2 starts at 3.6\,kW. A Swiss wall socket charges at \alert{2.3\,kW} --- in between.
  \item Charging without a wallbox is invisible to every published rule.
  \item We built the detector for that gap.
\end{itemize}
\column{.38\textwidth}
\centering\vspace{.2cm}
{\Large\textcolor{rose}{\textbf{AUC 0.49}}}\\[2pt]
{\small it finds hot-water boilers}\\[10pt]
{\scriptsize\textcolor{muted}{ramp rule: 0.686 \quad $n=342$}}
\end{columns}
\vspace{.45cm}
\rulebaseline{.95\linewidth}{At 15 minutes, a boiler and a granny-charger are the \emph{same signal}.}{\textbf{For AEW:} needs sub-minute data, or the boiler's tariff-relay channel}
\end{frame}
...
\begin{frame}{Rooftop PV: SolarPrint reads the solar imprint}
\begin{columns}[T,onlytextwidth]
\column{.54\textwidth}
...
\confusiontable{\shortstack[l]{GIGI PV\\(confirmed)}}{\shortstack[l]{Strictly\\unlabelled}}{260}{24}{4.6k}{65.6k}\\[4pt]
{\scriptsize\textcolor{muted}{$p \geq 0.5$; silver positives excluded.}}
\rulebaseline{.92\linewidth}{Flag PV if the meter ever exported. You cannot inject what you do not generate.}{F1 0.920 \enspace n = 342}
\confusiontable{\shortstack[l]{GIGI PV\\(confirmed)}}{\shortstack[l]{Unlabelled\\properties}}{259}{22}{23}{38}
\end{columns}
\end{frame}

\begin{frame}{Battery: a rule baseline, model to follow}
\begin{columns}[T,onlytextwidth]
\column{.54\textwidth}
...
\begin{itemize}\itemsep.10em
  ...
  \item The label-blind universe gate needs 95\% of known batteries admissible. Measured: \alert{93.7\%} (208/222).
  \item We \textbf{halted the pipeline} rather than weaken the gate to produce a number we could not defend.
\end{itemize}
\column{.41\textwidth}
\centering
\rulebaseline{.92\linewidth}{Of the solar exported in the shoulder months, what share leaves the house \emph{after} solar noon? Flag a battery above 0.50.}{ROC-AUC 0.855 \enspace n = 242}
\confusiontable{\shortstack[l]{GIGI battery\\(confirmed)}}{\shortstack[l]{Unlabelled\\PV homes}}{170}{20}{21}{31}\\[4pt]
{\scriptsize\textcolor{muted}{Scored within PV households.}}
\end{columns}
\end{frame}
